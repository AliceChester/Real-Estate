...
<?php
	...
	class Livraison{
		...
		function creer($ref){
			...
			if ($modules->charger_id($commande->transport))
			{
				$nom = $modules->nom;
				$nom[0] = strtoupper($nom[0]);

				if (@include_once("../client/plugins/" . $modules->nom . "/$nom.class.php"))
				{
					$tmpobj = new $nom();
	...
	function entete(){
		...
		$dateaff = substr($commande->datelivraison, 8, 2) . "/" . substr($commande->datelivraison, 5, 2) . "/" . substr($commande->datelivraison, 0, 4);
