...
<?php
	...
	function boucleRubrique($texte, $args){
		...
		$resul = CacheBase::getCache()->mysql_query($query, $rubrique->link);

		$compt = 1;

		if($resul=="" || count($resul)==0) return ""; else $nbres = count($resul);
		...
		foreach($resul as $row){
			...
			if($pasvide != ""){
				...
				$resul4 = CacheBase::getCache()->mysql_query($query4,$tmprod->link);
			...
			$resul3 = CacheBase::getCache()->mysql_query($query3, $rubrique->link);
	...
	function boucleImage($texte, $args){
        ...
        if($debut!="" && $num=="") $num= CacheBase::getCache()->mysql_query_count($query, $image->link);
	...
	function boucleDevise($texte, $args){
		...
		foreach($resul as $row) {
			...
			$temp = str_replace("#HTMLSYMBOLE", htmlentities($devise->symbole), $temp);
	...
	function boucleProduit($texte, $args, $type=0){
			...
			if($motcle){
				$motcle = strip_tags(mysql_real_escape_string($motcle));
				$liste="";

				$exp = "REGEXP '([[:<:]]${motcle})|(${motcle}[[:>:]])'";

  				$query = "
  					SELECT * FROM
  						$produitdesc->table
  					LEFT JOIN
  						$produit->table ON $produit->table.id=$produitdesc->table.produit
  					WHERE
  						$produit->table.ref='$motcle'
  					OR
  						titre $exp
  					OR
  						chapo $exp
		...
		$saveReq = str_replace("*", "count(id) as totcount", $saveReq);
		$saveRes = CacheBase::getCache()->mysql_query($saveReq,$produit->link);
		...
		foreach($resul as $row) {
			...
			if( $row->promo == "1" && $row->prix) $pourcentage =  round((100 * ($row->prix - $row->prix2)/$row->prix), 0);
	...
	function boucleContenu($texte, $args, $type=0){
				...
				$exp = "REGEXP '([[:<:]]${motcle})|(${motcle}[[:>:]])'";
  				...
  				$query = "
  					SELECT * FROM
  						$contenudesc->table
  					LEFT JOIN
  						$contenu->table ON $contenu->table.id=$contenudesc->table.produit
  					WHERE
  						$contenu->table.ref='$motcle'
  					OR
  						titre $exp
  					OR
  						chapo $exp
		...
		$saveRes = CacheBase::getCache()->mysql_query($saveReq, $contenu->link);
		$countRes = $saveRes[0]->totcount;
	...
	function bouclePanier($texte, $args){
      ...
      for($compt = 0; $compt<count($_SESSION['navig']->panier->tabarticle[$i]->perso); $compt++){
        ...
        $decval .= $val . " ";
			...
			$temp = str_replace("#TITRE", $_SESSION['navig']->panier->tabarticle[$i]->produitdesc->titre, $temp);
			$temp = str_replace("#QUANTITE", "$quantite", $temp);
			$temp = str_replace("#PRODUIT", $_SESSION['navig']->panier->tabarticle[$i]->produitdesc->produit, $temp);
	...
	function bouclePays($texte, $args){
		...
		$resul = CacheBase::getCache()->mysql_query($query, $pays->link);
		...
		foreach($resul as $row) {
			...
			if($pays->defaut == "1") $temp = str_replace("#DEFAULT", "selected=\"selected\"", $temp);
	...
	function boucleCaracdisp($texte, $args){
		...
		for($i=0; $i<count($tabliste); $i++){
            ...
            if($stockmini != ""){
				  ...
				  $resulch = CacheBase::getCache()->mysql_query($querych, $caracvalch->link);
	...
	function boucleCommande($texte, $args){
		...
		foreach($resul as $row) {
			...
			$datelivraison = $jour_livraison . "/" . $mois_livraison . "/" . $annee_livraison;
  			...
  			$resul2 = CacheBase::getCache()->mysql_query($query2, $venteprod->link);
  			$total = round($resul2[0]->total, 2);
	...
	function boucleDeclinaison($texte, $args){
		...
		if ($rubrique="") {
			$query = "
				SELECT
					dcd.declinaison as iddeclinaison, dcd.*
				FROM
					$tmpdeclinaison->table dcl
				LEFT JOIN
					$tmpdeclinaisondesc->table dcd on dcd.declinaison = dcl.id
				WHERE
					1 $search $order";
		} else {
			$query = "
				SELECT DISTINCT
					rub.declinaison as iddeclinaison, dcd.*
				FROM
					$rubdeclinaison->table rub
				LEFT JOIN
					$tmpdeclinaison->table dcl on dcl.id = rub.declinaison
				LEFT JOIN
					$tmpdeclinaisondesc->table dcd on dcd.declinaison = rub.declinaison
				WHERE
					1 $search $order";
		}
		...
		foreach($resul as $row) {
			...
			if ($stockmini > 0 && $produit > 0)
			{
				...
				$resdeclidisp = CacheBase::getCache()->mysql_query($query, $rubdeclinaison->link);

				if (empty($resdeclidisp)) continue;
			}

			$temp = str_replace("#ID", "$row->declinaison", $texte);
			$temp = str_replace("#TITRE", "$row->titre", $temp);
			$temp = str_replace("#CHAPO", "$row->chapo", $temp);
			$temp = str_replace("#DESCRIPTION", "$row->description", $temp);
			$temp = str_replace("#PRODUIT", "$produit", $temp);
