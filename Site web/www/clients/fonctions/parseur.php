...
<?php
	...
	function boucle_simple($lect, $boucles){
		...
		for($i=0; $i<count($boucles); $i++){
			...
			if(isset($liste[1])){
				...
				$args = $liste[1];
				$lect = preg_replace("|<THELIA_" . $boucles[$i] . " [^>]*>.*</THELIA_" . $boucles[$i] . ">|Us", boucle_exec($type_boucle, $args, $liste[2], $boucles[$i]), $lect, 1);
			}
		}
	...
	function boucle_exec($type_boucle, $args, $texte, $nom_boucle = ""){
		...
		$res = "";
			...
			modules_fonction( "avantboucle", $param);
			...
			modules_fonction( "apresboucle", $param);
