...
function ca_mois($mois, $annee, $jour, $pourc=100, $port=1){
      ...
      $query = "select * from $commande->table where statut>=2 and statut<>5 and datefact like '$date'";
...
include_once("../lib/artichow/LinePlot.class.php");
	...
	$graph->border->hide();
	$graph->title = new archiLabel("Progression du chiffre d'Affaires journalier. Total de ".getmonth($mois)." : ".ca_mois($mois, $annee, "%", 100, 1)." Euro;",new archiFileFont(ARTICHOW_FONT.'/Arial', 10));
