...
if (! is_array($cke_tags)) { $cke_tags = unserialize(_CKE_TAGS_DEF) ; }
...
function ckeditor_ecrire_protectedtags($tags=false) {
	...
	switch ($row['extension']) {
		...
		case 'png':
			if(!preg_match(',^\w+://,',$row['fichier'])) // s'il y a déjà un protocole, il ne faut pas ajouter le chemin vers $dir_img
				$row['fichier'] = url_absolue($row['fichier']) ;
			break ;
		default:
...
function ckeditor_fix_default_values() {
	...
	ecrire_config("ckeditor/protectedtags", ckeditor_lire_config("protectedtags", _CKE_PROTECTEDTAGS_DEF)) ; 
}
?>
