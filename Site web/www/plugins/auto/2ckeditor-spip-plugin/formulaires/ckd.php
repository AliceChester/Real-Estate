...
function formulaires_ckd_charger_dist() {
	...
	$valeurs = array(
		'utilise_kcfinder' => array_key_exists('utilise_kcfinder', $cfg)?$cfg['utilise_kcfinder']:_CKE_USE_KCFINDER_DEF,
		'utilise_kcfinderupload' => array_key_exists('utilise_kcfinderupload', $cfg)?$cfg['utilise_kcfinderupload']:_CKE_USE_DIRECTUPLOAD_DEF,
		...
		'base_dir' => array_key_exists('base_dir', $cfg)?$cfg['base_dir']:preg_replace(_CKE_RACINE_REGEX, '', _CKE_DIR_UPLOAD),
		'images_dir' => array_key_exists('images_dir', $cfg)?$cfg['images_dir']:_CKE_IMAGES_UPLOAD,
		'images_extensions_autorisees' => array_key_exists('images_extensions_autorisees', $cfg)?$cfg['images_extensions_autorisees']:_CKE_IMAGES_EXT_PARDEF,
		'flash_dir' => array_key_exists('flash_dir', $cfg)?$cfg['flash_dir']:_CKE_FLASH_UPLOAD,
		'flash_extensions_autorisees' => array_key_exists('flash_extensions_autorisees', $cfg)?$cfg['flash_extensions_autorisees']:_CKE_FLASH_EXT_PARDEF,
		'files_dir' => array_key_exists('files_dir', $cfg)?$cfg['files_dir']:_CKE_FILES_UPLOAD,
		'files_extensions_autorisees' => array_key_exists('files_extensions_autorisees', $cfg)?$cfg['files_extensions_autorisees']:_CKE_FILES_EXT_PARDEF
	) ;
	return $valeurs ;
}
...
function formulaires_ckd_traiter_dist() {
	if (_request('_cfg_delete')) {
		...
		ecrire_config("ckeditor/utilise_kcfinder", _request("utilise_kcfinder")) ;
		ecrire_config("ckeditor/utilise_kcfinderupload", _request("utilise_kcfinderupload")) ;
