 ...
 * Date :      Sun, 03 Apr 2011 15:43:49 GMT
 * Compile :   Wed, 26 Oct 2011 21:50:50 GMT
...
((!is_array($l = quete_logo('id_article', 'ON', $Pile[$SP]['id_article'],'', 0))) ? '':
 ("<img class=\"spip_logos\" alt=\"\" align=\"right\" src=\"$l[0]\"" . $l[2] .  ($l[1] ? " onmouseover=\"this.src='$l[1]'\" onmouseout=\"this.src='$l[0]'\"" : "") . ' />')),'150','150')))))!=='' ?
	...
	(($t2 = strval(interdire_scripts(texte_backend(traiter_doublons_documents($doublons, propre(nettoyer_chapo($Pile[$SP]['chapo']), $connect))))))!=='' ?
		...
		') :
			'') .
	(($t2 = strval(interdire_scripts(texte_backend(filtrer('image_graver', filtrer('image_reduire',traiter_doublons_documents($doublons, cs_nettoie(cs_decoupe(propre(cs_onglets($Pile[$SP]['texte']),$connect)))),'500','0'))))))!=='' ?
...
// Temps de compilation total: 18.320 ms
//
